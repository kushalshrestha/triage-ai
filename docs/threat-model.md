...
| 4 | Denial of service / cost abuse | Unbounded requests to the AI endpoints run up LLM API cost | In-memory per-user rate limiting on `POST /tickets/{id}/triage`, `POST /knowledge`, `GET /knowledge/search` (`app/rate_limit.py`) | Implemented — see ADR-0018 |
